...
export default function Following({params}) {
    ...
    const getUserFollows = async () => {
        const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/user/follow/${userId}/following`, {
            method: 'GET'
        });
    ...
    return (
        <main>
            ...
            <section className="flex flex-col items-center">
                <h2 className="text-2xl mt-8"><a href={`/user/${user.username}`} className="normal-link">{user.username}</a> follows:</h2>
                <section className="flex flex-col w-1/4 gap-4 mt-8">
                    {following.map((follow, index) => {
                        return (
                            <FollowedUser key={index} id_follow={follow.id_follow} id_followed={follow.id_followed} me={me} />
                        );
                    })}
                </section>
            </section>
